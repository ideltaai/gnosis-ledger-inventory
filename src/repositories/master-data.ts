...
export async function upsertRole(db: Db, organizationId: string, name: string, permissions: string[]) {
  const result = await db.query<{ id: string }>(
    `insert into roles (organization_id, name, permissions) values ($1, $2, $3)
     on conflict (organization_id, name) do update set permissions = excluded.permissions returning id`,
    [organizationId, name, permissions],
  );
  return result.rows[0].id;
}
