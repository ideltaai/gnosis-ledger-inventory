...
This repo is prepared for a single Railway app service plus one Railway PostgreSQL service. In production, `npm run build` builds the Vue app, and `npm start` runs the Node API on Railway's `PORT` while also serving `dist/` with SPA fallback for routes such as `/dashboard`, `/inventory`, and `/settings/users`.
...
See `docs/deployment/railway-production.md` for full setup, smoke tests, first-login steps, and backup notes.
